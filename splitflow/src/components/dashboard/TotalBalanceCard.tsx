...
const TotalBalanceCard = ({ totalBalance, posBalance, negBalance, membersCount, memberNames, monthlyChange = null, isLoading = false }: Props) => {
		...
		<Card className="p-6 sm:p-8">
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
				...
				<div className="min-w-0">
					...
					{/*  SKELETON FOR BIG NUMBER  */}
					{isLoading ? (
						<div className="flex items-center gap-3 mb-1.5 mt-2">
							<div className="h-10 sm:h-12 w-48 bg-slate-200 animate-pulse rounded-xl"></div>
						</div>
					) : (
					...
					{isLoading ? (
						<div className="h-4 w-48 bg-slate-100 animate-pulse rounded mt-3"></div>
					) : (
				...
				<div className="flex gap-3 shrink-0">
					<div className="bg-teal-50 rounded-xl px-5 py-4 border border-teal-100/80 min-w-30">
						...
						{/*  SKELETON FOR OWED AMOUNT  */}
						{isLoading ? (
							<div className="h-7 w-20 bg-teal-200/60 animate-pulse rounded-md mt-0.5"></div>
						) : (
					...
					<div className="bg-orange-50 rounded-xl px-5 py-4 border border-orange-100/80 min-w-30">
						...
						{/*  SKELETON FOR OWE AMOUNT  */}
						{isLoading ? (
							<div className="h-7 w-20 bg-orange-200/60 animate-pulse rounded-md mt-0.5"></div>
						) : (
			...
			<div className="pt-4 border-t border-slate-100 flex items-center justify-between">
				...
				<div className="flex items-center -space-x-2">
					...
					{isLoading ? (
						<>
							<div className="w-7 h-7 rounded-full bg-slate-200 animate-pulse ring-2 ring-white z-30"></div>
							<div className="w-7 h-7 rounded-full bg-slate-200 animate-pulse ring-2 ring-white z-20"></div>
							<div className="w-7 h-7 rounded-full bg-slate-200 animate-pulse ring-2 ring-white z-10"></div>
						</>
					) : (
						<>
							...
							{membersCount === 0 && (
								<span className="text-xs text-slate-400 ml-2">No recent interactions</span>
							)}
						</>
					)}
				</div>
